...
- Some of the names in our `Brand Name` column look very similar. Before we spend a lot of time trying to manually clean or cluster all 1781 of them let’s try two easy cleaning steps.
- First, let’s trim any whitespace at the beginning or end of the row on the `Brand Name` column by selecting `edit cells` > `common transforms` > `trim leading and trailing whitespace` – already our total number has come down to 1778
- Next, let’s change everything to titlecase by selecting `edit cells` > `common transforms` > `To titlecase` – now we have 1693 options.
...
- Start with the `Key Collision` Method and work your way through the `keying functions` – there should be many options to cluster. If they all look good you can use `select all` in the bottom left (note that you might need to decrease the size of your window to see this)
...
- Under `DSLD ID` select `Sort   and we want to sort by number. Note that we can position errors and blanks at the beginning to help us spot them if we want.
    + You can edit your sort by selecting `Sorts under the specific column, or reversing or removing it to undo (since it is not listed in the `Undo/redo` column).
